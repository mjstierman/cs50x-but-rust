...
Next execute `wget https://cdn.cs50.net/2024/fall/psets/4/filter-less.zip` in order to download a ZIP called filter-less.zip into your codespace.

Then execute `unzip filter-less.zip` to create a folder called filter-less. You no longer need the ZIP file, so you can execute

rm filter-less.zip and respond with “y” followed by Enter at the prompt to remove the ZIP file you downloaded.

Now type `cd filter-less` followed by Enter to move yourself into (i.e., open) that directory. Your prompt should now resemble the below.
